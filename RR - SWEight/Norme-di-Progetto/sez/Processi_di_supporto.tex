...
\item Finita l'attività di scrittura crea un pull request con (Needs review):
	\begin{enumerate}
		\item[3.1] Sé stesso come assigneer;
		\item[3.2] I verificatori come reviewers;
		\item[3.3] I campi project e milestone vuoti.
	\end{enumerate} 
\item I verificatori controllano il documento e possono:
	\begin{enumerate}
	 	\item[4.1] Accettarlo (Reviewer approved);
	 	\item[4.2] Non accettarlo (In progress).
	\end{enumerate}
...
\begin{enumerate}
	\item[6.1] Approvarlo, rendendo il documento formale e chiudendo la issue. (Done)
	\item[6.2] Non approvarlo, fornendo le opportune motivazioni (In progress);
\end{enumerate}
