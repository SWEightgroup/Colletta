...
Tutti i requisiti devono essere riferiti al componente che li soddisfa per poter verificare che ogni requisito sia soddisfatto. Si veda le sezione \ref{sec:Trac} in cui viene descritto lo strumento utilizzato per il tracciamento.

\paragraph{Tracciamento delle classi}\mbox{}\\
Tutti i requisiti devono essere tracciati alle classi associate per poter verificare che ogni classe soddisfi almeno un requisito. Si veda le sezione \ref{sec:Trac} in cui viene descritto lo strumento
...
Il tracciamento dei requisiti, delle componenti e delle classi avviene attraverso la piattaforma \textit{PragmaDB}, installata su un server {AWS}\ped{G} ad hoc per il gruppo \gruppo . \textit{PragmaDB} è un tool sviluppato dal gruppo Pragma durante il progetto di ingegneria del software nell'AA 2014/2015 e liberamente disponibile su GitHub. Punto di forza di questo pacchetto software sono la specificità per il progetto e l'esportazione dei dati in LaTex.
