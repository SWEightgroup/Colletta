...
\newpage
\section{F}
\textbf{Feature branch}:\\Permette agli sviluppatori di create e testare le nuove funzionalità  prima di essere aggiunte al prodotto software in sviluppo.
...
\textbf{Integration big-bang}:\\Il test di integrazione è la fase di test del software in cui i singoli moduli software vengono combinati e testati come gruppo. Si verifica dopo il collaudo dell'unità e prima dei test di convalida. I test di integrazione prendono come moduli di input che sono stati testati da unità,  li raggruppano in aggregati più grandi, applicano test definiti in un piano di test di integrazione a tali aggregati e forniscono come output il sistema integrato pronto per il test del sistema. 
\\Nell'approccio big-bang, la maggior parte dei moduli sviluppati sono accoppiati per formare un sistema software completo o una parte importante del sistema e quindi utilizzati per i test di integrazione. Questo metodo è molto efficace per risparmiare tempo nel processo di test di integrazione. Tuttavia, se i casi di test ed i loro risultati non sono registrati correttamente, l'intero processo di integrazione sarà più complicato e potrebbe impedire al team di test di raggiungere l'obiettivo dei test di integrazione.
